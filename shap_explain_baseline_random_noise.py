...
from util.custom_functions import collect_perf_metrics, awgn
...
def main():
    ...
    for snr in snr_list:
        ...
        global feature_map_hash_disct
        feature_map_hash_disct = {}

        technique_name = tech.format(snr)
        # ------------ Util/Priv performance paths ----------------
        util_sv_perf_path = f'./data/nn_obfuscator_perf/sv_privacy/util_sv_tec_{technique_name}.xlsx'
        priv_emo_perf_path = f'./data/nn_obfuscator_perf/sv_privacy/priv_emo_tec_{technique_name}.xlsx'
        priv_gen_perf_path = f'./data/nn_obfuscator_perf/sv_privacy/priv_gen_tec_{technique_name}.xlsx'
        ...
        for index, top_k_size in tqdm(enumerate(top_k_sizes)):
            ...
            if not Path(util_sv_perf_path).exists() or not Path(priv_emo_perf_path).exists():
                ...
                util_sv_perf_list.append(util_sv_perf[0])
                priv_e_perf_list.append(priv_e_perf[0])
                priv_g_perf_list.append(priv_g_perf[0])
...
def train_obfuscator_top_k_features(shap_data_dict, topk_size, x_train, x_test, util_labels, priv_e_labels,
    ...
    if len(model_features) == 0:
        return [], [], [], [], []
    elif features_map_hash in feature_map_hash_disct:
        return [9999], [9999], [9999], [9999], [9999]
    else:
        feature_map_hash_disct[hash(model_features.data.tobytes())] = None
    ...
    for _ in tqdm(range(30)):
        # Creating noise mask for private top k features
        noise = awgn(x_test, snr)
        noisy_x_test = x_test.copy()
        noisy_x_test[:, model_features] += noise[:, model_features]

        util_sv_perf, priv_e_perf, priv_g_perf = evaluate_technique(noisy_x_test,
                                                                    util_labels,
                                                                    priv_e_labels,
                                                                    priv_g_labels)
    ...
    util_sv_f1 = np.mean(util_sv_perf_list)
    util_emo_f1 = np.mean(priv_e_perf_list)
    util_gen_f1 = np.mean(priv_g_perf_list)

    return [util_sv_f1], [util_emo_f1], [util_gen_f1], priv_features, topk_size
...
if __name__ == "__main__":
    ...
    obf_model_meta_data = 'obf_checkpoint/model_obf_meta_k_removal_noise_{}_util5'

    tech = "noise_gamma_{}_private_protect_util"
