...
from experiment_neural_nets import build_sv_model_swish
...
def main():
    ...
    emo_model_path = 'sv_model_checkpoint/sver_model_scalarized_{}.h5'.format(db_name)

    print("Pre-processing audio files!")
    x_train_mfcc, x_test_mfcc, _, _, _, _, y_id_train_encoded, y_id_test_encoded = pre_process_data(audio_files_path,
                                                                                                    db_name)
    ...
    model = build_sv_model_swish(x_train_cnn_scaled)
    model_path = emo_model_path

    y_train = y_id_train_encoded
    y_test = y_id_test_encoded
